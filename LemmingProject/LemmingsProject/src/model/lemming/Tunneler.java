...
public class Tunneler extends Observable implements StateOfLemming{
	...
	public void move() {
		...
		if(!active){
			if(lemming.getGame().isOut(lastDown) )  // if lastDown is out lemming die
				...
				if(lemming.getAltitudeCounter() >=5) lemming.die();
			...
			else if( lemming.testReproducer(next)){  // if next = Reproducer
				active = true;
				lemming.setContitionToChangeType(false);
			}
		
			else if( lemming.testExploder(next)){ // if next = Exploder
				active = true;
				lemming.setContitionToChangeType(false);
			}
		
			else if( lemming.testDestructible(next)){ // if next = Destructible   destroy the block and next = next
				active = true;
				lemming.setContitionToChangeType(false);
			}
		...
		if(active){
			if( lemming.testReproducer(next)){  // if next = Reproducer
				...
				lemming.activeExploder(next);
				lemming.die();
			}
			
			else if( lemming.testDestructible(next)){ // if next = Destructible   destroy the block and next = next
				lemming.destroyBlock(next);
				lemming.setContitionToChangeType(false);
		...
		if( !lastDown.equals(Coordinate.getEndCoordinate(lemming.getGame() )) && lemming.getAltitudeCounter() >=5 && !lemming.testVoid(lastDown)) 
			...
			if(!lemming.isContitionToChangeType() && !active && !lemming.ifArrived() && lemming.ifAlive() ){
				...
				lemming.addChange(last,Change.ChangeType.VOID);
				lemming.addChange(next,Change.ChangeType.TUNNELER);
				lemming.setBody(next);
			}
